...
async def setup_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        "Per-user Google Sheets setup\n\n"
        "Each Telegram user connects their own spreadsheet.\n\n"
        "Steps:\n"
        "1. Google Cloud Console → IAM & Admin → Service Accounts → "
        "Create (or pick) a service account → Keys → Add key → JSON.\n"
        "2. Open the JSON, copy `client_email`, and share your spreadsheet with that "
        "address (Editor).\n"
        "3. In Telegram, send this JSON file to the bot as a file attachment "
        "(paperclip → File). Do not paste the private key in chat.\n"
        "4. Run /setsheet with the exact spreadsheet title as shown in Google Sheets, e.g.:\n"
        "   /setsheet My Calorie Log\n\n"
        "Optional later:\n"
        "• Re-send a new JSON to rotate credentials (you will need /setsheet again)."
    )
